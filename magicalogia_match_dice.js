        ...
        try {
            if (obj.get('subtype') == 'card') {
                ...
                if (model) {
                    ...
                    let dice = findObjs({ _type: 'graphic', _subtype: 'card', layer: 'objects'});
...
if (msg.type == "api"){
    if (msg.content.indexOf("!match_dice") === 0) {
        ...
        if (dice[0].length < 1 && dice[2].length < 1) {
            sendChat('error','/w GM **"+getObj('page',Campaign().get("playerpageid")).get('name')+"** 페이지의 대표 플롯 영역 내에 공개된 다이스가 없습니다. 현재 다이스 플롯상태: 대표A:'
